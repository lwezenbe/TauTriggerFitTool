...
class fittingTool():
	...
	def getValuesAroundParam(self, param_index):
		...
		self.p0values = np.linspace(min(0.01, param[0]-1.), param[0]+1., 10)	
		self.p1values = np.linspace(min(1.01, param[1]-1.5), param[1]+1.5, 12)	
		self.p2values = np.linspace(min(0.01, param[2]-2.5), param[2]+2.5, 10)	
		self.p3values = np.linspace(min(-5., param[3]-10.), param[3]+10., 10)	
	...
	def setFitParam(self, ntry):
                ...
                self.f.FixParameter( 5, self.param[ntry][5])
	...
	def performFit(self, second_try = False):
		...
		for ntry in xrange(len(self.param)):
			...
			if int(fit_result) == 0 and fit_result.CovMatrixStatus() == 3:       
		...
		if second_try:
			...
			if initial_step_converged:	
				return fit_result
			else:					#Try to look in the neighbourhood of initial estimate that gave the best fit
